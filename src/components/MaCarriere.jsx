...
// 👇 NOUVEAU : On ajoute "onMatchClick" dans les propriétés
export default function MaCarriere({ userProfile, tournaments, onMatchClick }) {
  ...
  const { careerStats, matchHistory } = useMemo(() => {
    ...
    if ((!currentUserId && !currentUserName) || !tournaments) {
        return { careerStats: null, matchHistory: [] };
    }
    ...
    tournaments.forEach(t => {
      ...
      const myLocalIds = [];
      if (currentUserId) myLocalIds.push(currentUserId);

      if (t.teams) {
        t.teams.forEach(team => {
          team.players?.forEach(p => {
            if (currentUserId && (p.profile_id === currentUserId || p.user_id === currentUserId || p.player_id === currentUserId || p.id === currentUserId)) {
              if (p.id && !myLocalIds.includes(p.id)) myLocalIds.push(p.id);
            }
          });
        });
      }
      ...
      allMatches.forEach(m => {
        ...
        const isStrictMe = (p) => {
            if (!p) return false;
            if (p.id && myLocalIds.includes(p.id)) return true;
            if (currentUserId && (p.profile_id === currentUserId || p.user_id === currentUserId || p.player_id === currentUserId || p.id === currentUserId)) return true;
            return false;
        };

        const isNameMe = (p) => {
            if (!p) return false;
            return currentUserName && currentUserName.trim() !== "" && p.name === currentUserName;
        };
        ...
        let pInA = m.savedStatsA?.find(isStrictMe);
        let pInB = m.savedStatsB?.find(isStrictMe);

        if (!pInA && !pInB) {
            pInA = m.savedStatsA?.find(isNameMe);
            if (!pInA) { 
                pInB = m.savedStatsB?.find(isNameMe);
            }
        }
        ...
        if (pInA) {
            ...
            myTeam = m.teamB;
            oppTeam = m.teamA;
            isWin = (m.scoreB || 0) > (m.scoreA || 0);
            myScore = m.scoreB || 0;
            oppScore = m.scoreA || 0;
        }

        if (myStats && (myStats.timePlayed > 0 || myStats.points > 0 || myStats.fouls > 0)) {
            gp++;
            ...
            const missedShots = ((myStats.fta || 0) - (myStats.ftm || 0)) + 
                               ((myStats.fg2a || 0) - (myStats.fg2m || 0)) + 
                               ((myStats.fg3a || 0) - (myStats.fg3m || 0));
            const matchEff = (myStats.points || 0) + matchReb + (myStats.ast || 0) + (myStats.stl || 0) + (myStats.blk || 0) 
                             - (myStats.tov || 0) - missedShots - (myStats.fouls || 0);
            totalEff += matchEff;
            ...
            history.push({
                id: m.id + "-" + Math.random(),
                tourneyName: t.name || 'Tournoi',
                tourneyId: t.id, // 👈 On garde l'ID du tournoi pour ouvrir la feuille
                originalMatch: m, // 👈 On garde le match complet pour l'ouvrir
                teamName: myTeam?.name || 'Individuel',
                oppTeamName: oppTeam?.name || 'Adversaire',
                isWin,
    ...
    return { careerStats: stats, matchHistory: history }; 
  }, [tournaments, currentUserId, currentUserName, activeFormat]);
        ...
        <div className="relative z-10 flex-1 flex flex-col">
          ...
          {!careerStats || careerStats.gp === 0 ? (
            ...
            <>
              ...
              {/* 👇 NOUVEAU : Grille (2 colonnes) et limitation à 4 éléments maximum */}
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 pb-4">
                  {matchHistory.slice(0, 4).map((m, i) => (
                      <div 
                        key={i} 
                        onClick={() => onMatchClick && onMatchClick(m.originalMatch, m.tourneyId)}
                        className={`flex flex-col sm:flex-row items-start sm:items-center justify-between bg-app-card border border-muted-line p-4 sm:p-5 rounded-2xl hover:border-muted transition-all gap-4 group shadow-lg ${onMatchClick ? 'cursor-pointer hover:-translate-y-1 hover:bg-white/5 hover:shadow-xl' : ''}`}
                      >
                          ...
                          <div className="flex items-center gap-4 w-full sm:w-auto">
                              ...
                              <div className="flex flex-col flex-1">
                                  <div className="text-white font-black text-base sm:text-lg tracking-wide group-hover:text-secondary-light transition-colors truncate max-w-[200px] sm:max-w-none">
                                      {m.teamName} <span className="text-muted text-xs sm:text-sm mx-1 sm:mx-1.5 font-medium italic">vs</span> {m.oppTeamName}
                                  </div>
                                  <div className="text-muted-light text-[9px] sm:text-[11px] font-black tracking-widest uppercase mt-1.5 flex flex-wrap items-center gap-1 sm:gap-2">
                                      <span className="text-secondary drop-shadow-md truncate max-w-[120px] sm:max-w-[150px]">🏆 {m.tourneyName}</span>
                                      <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-muted-dark hidden sm:block"></span>
                                      <span className="bg-app-input px-1.5 sm:px-2 py-0.5 rounded border border-muted-line text-muted whitespace-nowrap">Score: <b className="text-white">{m.finalScore}</b></span>
                                  </div>
                              </div>
                          </div>
                          
                          <div className="flex gap-2 sm:gap-4 bg-app-input px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-muted-line w-full sm:w-auto justify-between sm:justify-start shadow-inner shrink-0">
                              <div className="flex flex-col items-center min-w-[35px] sm:min-w-[40px]">
                                  <span className="text-[9px] sm:text-[10px] text-muted-dark font-black tracking-widest">PTS</span>
                                  <span className="text-white font-black text-xs sm:text-sm">{m.pts}</span>
                              </div>
                              <div className="flex flex-col items-center min-w-[35px] sm:min-w-[40px]">
                                  <span className="text-[9px] sm:text-[10px] text-muted-dark font-black tracking-widest">REB</span>
                                  <span className="text-white font-black text-xs sm:text-sm">{m.reb}</span>
                              </div>
                              <div className="flex flex-col items-center min-w-[35px] sm:min-w-[40px]">
                                  <span className="text-[9px] sm:text-[10px] text-muted-dark font-black tracking-widest">AST</span>
                                  <span className="text-white font-black text-xs sm:text-sm">{m.ast}</span>
                              </div>
                              <div className="flex flex-col items-center min-w-[35px] sm:min-w-[40px] pl-2 sm:pl-4 border-l border-muted-line">
                                  <span className="text-[9px] sm:text-[10px] text-secondary font-black tracking-widest">ÉVAL</span>
                                  <span className="text-secondary font-black text-xs sm:text-sm drop-shadow-md">{m.eff}</span>
                              </div>
                          </div>
