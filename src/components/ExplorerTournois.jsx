      ...
      publicTourneys.map(t => {
        ...
        return (
          <div key={t.id} className="bg-[#1e1e2a] p-5 rounded-xl border border-white/5 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-purple-500/30 hover:shadow-[0_8px_25px_rgba(168,85,247,0.15)] group">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <span className="text-[0.65rem] text-[#888] flex items-center gap-1.5 font-bold bg-black/30 px-2 py-1 rounded-md border border-white/5">
                📅 {t.date || 'Date non définie'}
              </span>
              
              {/* BADGE TYPE DE TOURNOI (Remplace TeamTagList) */}
              <span className="text-[0.65rem] text-purple-400 flex items-center gap-1.5 font-black bg-purple-500/10 px-2 py-1 rounded-md border border-purple-500/20 tracking-widest">
                🏀 {formatLabel}
              </span>
            </div>
            ...
            {myCaptainTeams.length === 0 && courtSize !== 1 ? (
              ...
              <button 
                ...
                className="w-full mt-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-3 rounded-xl font-black tracking-widest text-sm shadow-[0_4px_15px_rgba(168,85,247,0.3)] hover:shadow-[0_6px_20px_rgba(168,85,247,0.5)] transition-all hover:-translate-y-0.5 cursor-pointer active:scale-95"
              >
                ...
                return (
                  <div key={t.id} onClick={() => handleOpenTourney(t.id)} className="bg-[#1e1e2a] p-5 rounded-xl border border-white/5 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-blue-500/40 hover:shadow-[0_8px_25px_rgba(59,130,246,0.15)] group cursor-pointer">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      ...
                      <span className="text-[0.65rem] text-blue-400 flex items-center gap-1.5 font-black bg-blue-500/10 px-2 py-1 rounded-md border border-blue-500/20 tracking-widest uppercase">
                ...
                return (
                  <div key={t.id} onClick={() => handleOpenTourney(t.id)} className="bg-[#1e1e2a] p-5 rounded-xl border border-white/5 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-emerald-500/40 hover:shadow-[0_8px_25px_rgba(16,185,129,0.15)] group cursor-pointer relative overflow-hidden">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4 pl-2">
                      ...
                      <span className="text-[0.65rem] text-emerald-400 flex items-center gap-1.5 font-black bg-emerald-500/10 px-2 py-1 rounded-md border border-emerald-500/20 tracking-widest uppercase">
                        🏀 {courtSize}X{courtSize}
                      </span>
                    </div>
        ...
        <div className="bg-[#15151e]/60 backdrop-blur-md rounded-2xl p-5 border border-white/5 flex flex-col shadow-2xl relative overflow-hidden group/col">
          ...
          <div className="flex bg-black/40 rounded-lg p-1 mb-6 border border-white/5">
            <button onClick={() => setFilterFinished('all')} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${filterFinished === 'all' ? 'bg-[#333] text-white shadow-md' : 'text-[#666] hover:text-[#aaa]'}`}>Tous</button>
            <button onClick={() => setFilterFinished('mine')} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${filterFinished === 'mine' ? 'bg-emerald-600/30 text-emerald-400 shadow-md' : 'text-[#666] hover:text-[#aaa]'}`}>Mes matchs</button>
          </div>
              ...
              finishedTourneys.map(t => {
                ...
                return (
                  <div key={t.id} onClick={() => handleOpenTourney(t.id)} className="bg-[#1e1e2a]/50 p-5 rounded-xl border border-white/5 cursor-pointer transition-all duration-300 hover:bg-[#1e1e2a] hover:-translate-y-1 hover:border-[#444] group">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      <span className="text-[0.65rem] text-[#666] flex items-center gap-1.5 font-bold bg-black/20 px-2 py-1 rounded-md border border-white/5">
                        📅 {t.date || 'Terminé'}
                      </span>
                      <span className="text-[0.65rem] text-gray-400 flex items-center gap-1.5 font-black bg-white/5 px-2 py-1 rounded-md border border-white/10 tracking-widest uppercase grayscale group-hover:grayscale-0 transition-all">
                        🏀 {courtSize}X{courtSize}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-black/30 p-3 rounded-lg mt-4 border border-white/5">
                      ...
                      {iParticipated && (
                        <span className="text-[9px] bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 px-2 py-1.5 rounded font-black uppercase tracking-widest shadow-sm">J'y étais !</span>
                      )}
                    </div>
                  </div>
                );
