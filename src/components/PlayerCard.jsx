...
const PlayerCard = React.memo(({ team, player, onPlayerClick, pendingSubs, pendingAction, onConfirm, hasGlobalAction, pendingAssist, activeActionType, canEdit, pendingFoul, isForcedSub, maxFouls, courtSize }) => {
  ...
  const baseStatusClasses = player.status === 'bench' 
    ...
    : `opacity-100 grayscale-0 border ${team === 'A' ? 'border-secondary shadow-[0_4px_12px_rgba(249,115,22,0.1)]' : 'border-action shadow-[0_4px_12px_rgba(59,130,246,0.1)]'}`;

  // 2. Superposition des effets prioritaires
  let priorityClasses = '';
  
  if (isPendingFoul) {
    priorityClasses = 'ring-4 ring-danger bg-danger/20 shadow-[0_0_25px_rgba(239,68,68,0.6)] z-10 scale-105';
  } else if (isMustLeave) {
    // ALERTE VISUELLE : Le joueur fautif clignote en rouge vif
    priorityClasses = 'ring-4 ring-danger bg-danger/30 shadow-[0_0_30px_rgba(239,68,68,0.8)] z-10 animate-pulse';
  } else if (isPendingScore) {
    priorityClasses = 'ring-4 ring-white bg-white/10 z-10 scale-105';
  } else if (isSubSelected) {
    priorityClasses = 'ring-4 ring-purple-500 bg-purple-500/20 z-10 scale-105'; // J'ai gardé le violet natif de tailwind pour la sélection SUB
  ...
  // bg-app-panel remplace #222
  const finalClasses = `relative bg-app-panel rounded-lg p-2 flex flex-col justify-between overflow-hidden transition-all duration-300 ease-in-out select-none ${baseStatusClasses} ${priorityClasses}`;
  ...
  return (
    ...
    >
      {isExcluded && <div className="absolute top-0 right-0 bg-danger text-white text-[0.6rem] font-bold px-1 py-0.5 rounded-bl-md z-10">{excluReason}</div>}
      
      {/* LIGNE 1 : Numéro, Nom, Temps */}
      <div className="flex justify-between items-center mb-1">
        <span className="text-xs font-black text-muted bg-black px-1.5 py-0.5 rounded">{player.number}</span>
        <span className="text-sm font-bold text-white truncate max-w-[80px]" title={player.name}>{player.name}</span>
        <span className="text-[0.65rem] font-bold text-muted">{`${Math.floor(player.timePlayed / 60).toString().padStart(2, '0')}:${(player.timePlayed % 60).toString().padStart(2, '0')}`}</span>
      </div>

      {/* LIGNE 2 : Points et Fautes (Séparés sur deux lignes) */}
      <div className="flex flex-col items-center bg-app-bg rounded py-1.5 mb-1 gap-1.5">
        
        {/* LIGNE 2A : Les Points au centre */}
        <span className="text-xl font-black text-white leading-none">
          {player.points} <span className="text-[0.65rem] text-muted font-bold ml-0.5">PTS</span>
        </span>
              ...
              return (
                <div 
                  ...
                  className={`w-[16px] h-[16px] flex items-center justify-center rounded-[3px] text-[10px] font-normal transition-colors ${
                    isFilled 
                      ? (isDanger 
                          ? 'bg-danger border border-danger text-white shadow-[0_0_5px_rgba(239,68,68,1)]' 
                          : 'bg-white border border-white text-black') 
                      : 'bg-app-bg border border-muted-dark text-transparent'
                  }`}
                >
                  {isFilled ? foulLetter : ''}
                </div>
      ...
      <div className="flex justify-center gap-2 flex-wrap text-[0.65rem] font-bold text-muted-light mt-1">
        {minorStats.length === 0 ? (
          <span className="text-muted-dark">Aucune stat</span>
        ) : (
      ...
      {isPendingScore && canEdit && (
        <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center gap-2 z-20 backdrop-blur-md px-3" onClick={e => e.stopPropagation()}>
          ...
          <button 
            className="w-full py-2 rounded border border-primary bg-primary/20 text-primary-light font-black text-xs tracking-widest uppercase hover:bg-primary hover:text-black transition-all cursor-pointer shadow-[0_0_10px_rgba(16,185,129,0.2)]" 
          ...
          <button 
            className="w-full py-2 rounded border border-danger bg-danger/20 text-danger font-black text-xs tracking-widest uppercase hover:bg-danger hover:text-white transition-all cursor-pointer shadow-[0_0_10px_rgba(239,68,68,0.2)]" 
